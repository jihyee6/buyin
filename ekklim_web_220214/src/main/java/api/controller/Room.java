...
@WebServlet("/room/*")
public class Room extends CommonAPI {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		
		...
		String room_id = request.getRequestURI().substring(request.getContextPath().length()+request.getServletPath().length()+1);
...
//		if (method.equals("GET")) {
		if (true) {
			...
			JSONObject info = imageDAO.searchRoomImage(room_id);
			String id = (String) session.getAttribute("id");
			
//			System.out.println("lodgement_id: " + info.get("lodgement_id"));
			
			try {
				
				//http://localhost:8081/ekklim_web_220214
				String search_url = "https://www.buyinhotel.co.kr/room/" + info.get("lodgement_id");
//				String web_url = "https://www.buyinhotel.co.kr/reserhotel/reservationhotel?hotel=" + info.get("lodgement_id") + "&checkin="+ checkin + "&checkout=" + checkout; 
				String web_url = "http://ekklim.kr/reserhotel/reservationhotel?hotel=" + info.get("lodgement_id") + "&checkin="+ checkin + "&checkout=" + checkout; 
